...
-- Unidades de negócio da cooperativa multisetorial (exemplo)
insert into public.business_units (id, company_id, name, code) values
  ('b0000000-0000-0000-0000-0000000000a1','a0000000-0000-0000-0000-000000000001','Posto de Combustível','POSTO'),
  ('b0000000-0000-0000-0000-0000000000a2','a0000000-0000-0000-0000-000000000001','Loja Agropecuária','AGRO'),
  ('b0000000-0000-0000-0000-0000000000a3','a0000000-0000-0000-0000-000000000001','Supermercado','SUPER')
on conflict (id) do nothing;
...
-- ---------- Usuários ----------
-- ATENÇÃO: a criação de usuários foi REMOVIDA daqui de propósito.
-- Inserir direto em auth.users por SQL funciona no Supabase local, mas o serviço de
-- Auth da nuvem (GoTrue) valida colunas que mudam entre versões — o usuário aparece
-- na tabela e mesmo assim o login é recusado.
--
-- Caminho correto:
--   1. Painel → Authentication → Users → Add user (marque "Auto Confirm User")
--   2. Rode supabase/usuarios.sql para atribuir papéis e setores
--
-- As demandas de exemplo abaixo só são criadas se já existir um usuário
-- com e-mail requisitante@compraflow.local.

-- ---------- Demandas demo ----------
do $$
declare v_req uuid := coalesce(
          (select id from auth.users where email='requisitante@compraflow.local'),
          (select id from auth.users order by created_at limit 1));  -- fallback: 1o usuario
