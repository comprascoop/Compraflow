...
export interface NewRequestInput {
  title: string; department_id: string; cost_center_id: string; category_id: string | null;
...
export async function createRequest(input: NewRequestInput): Promise<string> {
  ...
  const { data: req, error } = await supabase.from("purchase_requests").insert({
    title: input.title, department_id: input.department_id, cost_center_id: input.cost_center_id,
...
export async function transition(requestId: string, to: RequestStatus, comment?: string) {
  const { error } = await createClient().schema("app").rpc("fn_transition_request", {
    p_request_id: requestId, p_to_status: to, p_comment: comment ?? null,
  });
  if (error) throw error;
}
export async function assignBuyer(requestId: string) {
  const { error } = await createClient().schema("app").rpc("fn_assign_buyer", { p_request_id: requestId });
  if (error) throw error;
}
...
export async function generateOrders(requestId: string): Promise<number> {
  const { data, error } = await createClient().schema("app").rpc("fn_generate_orders", { p_request_id: requestId });
